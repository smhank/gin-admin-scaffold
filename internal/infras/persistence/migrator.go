...
func newMigrator(db *gorm.DB) *migrator {
	...
	// 获取当前最大批次号
	var maxBatch int
	db.Model(&model.Migration{}).Select("COALESCE(MAX(batch), 0)").Scan(&maxBatch)

	return &migrator{
		db:    db,
		batch: maxBatch + 1,
		migrations: []migration{
			{Name: "20250506_init_seed_data", Run: seedInitData},
			{Name: "20250506_add_operation_log_menu", Run: seedOperationLogMenu},
		},
	}
}
...
// seedInitData 初始化种子数据
func seedInitData(db *gorm.DB) error {
	// 检查是否已有数据（兼容旧版 SeedData 已导入的数据）
	var userCount int64
	...
	menus := []model.Menu{
		...
		{Name: "角色管理", Icon: "User", Path: "/roles", ParentID: func() *uint { id := uint(2); return &id }(), Sort: 1, Status: 1},
		{Name: "菜单管理", Icon: "Menu", Path: "/menus", ParentID: func() *uint { id := uint(2); return &id }(), Sort: 2, Status: 1},
		{Name: "API管理", Icon: "Connection", Path: "/apis", ParentID: func() *uint { id := uint(2); return &id }(), Sort: 3, Status: 1},
		{Name: "用户管理", Icon: "UserFilled", Path: "/users", ParentID: func() *uint { id := uint(2); return &id }(), Sort: 4, Status: 1},
	}
	for i := range menus {
...
// seedOperationLogMenu 添加操作历史菜单
func seedOperationLogMenu(db *gorm.DB) error {
	// 检查菜单是否已存在
	var count int64
